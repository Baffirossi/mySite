...
<?php
		...
		$support = '.row-fluid .content-blocks	{
			float: left;
			width: 23%;
			margin-top: 20px;
			margin-left: 15px;
...
';
		...
		$listen = '.row-fluid .content-blocks	{
			float: left;
			width: 29%;
			margin-top: 20px;
			margin-left: 22px;
...
';
		...
		$contact = 'input:not(#email_address), textarea, select	{
			border: 1px solid;
...
';
		...
		$wish = 'input:not(#email_address), textarea, select	{
			border: 1px solid;
			background-color: lightgrey;
			}
';

		$advertising = 'input:not(#email_address), textarea, select	{
			border: 1px solid;
			background-color: lightgrey;
			}
				label	{
...
';
       ...
       $currentpage = $_SERVER['REQUEST_URI'];
		if($currentpage=="/about/friends-of-the-chorus/") 
		{	
			echo $support;
		}
		if($currentpage=="/about/listen/") 
		{	
			echo $listen;
		}
		if($currentpage=="/contact-us/" or "/wfm/") 
		{	
			echo $contact;
		}
		if($currentpage=="/well-wish-order/") 
		{	
			echo $wish;
		}
		if($currentpage=="/advertising/") 
		{	
			echo $advertising;
		}
         ?>
...
<?php echo $smof_data['matrix_custom_css']; ?>
</style>
</head>
<body <?php body_class(); ?>>
...
<!-- END LOGO -->
<div id="header-widgets" class="header-widget">
